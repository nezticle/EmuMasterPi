...
static inline void sound_copy(s16 *stream_base, int length) {
	for (int i = 0; i < length; i++) {
		s32 current_sample = sound_buffer[sound_buffer_base + i];
		if (current_sample > 2047)
			current_sample = 2047;
		if (current_sample < -2048)
			current_sample = -2048;
		stream_base[i] = current_sample << 4;
		sound_buffer[sound_buffer_base + i] = 0;
	}
}
...
int GbaSpu::fillBuffer(char *stream, int length) {
	...
	if ((sound_buffer_base + sample_length) >= SoundBufferSize) {
		...
		sound_copy((s16 *)stream + partial_length, remainder_length);
		sound_buffer_base = remainder_length;
	} else {
		sound_copy((s16 *)stream, sample_length);
